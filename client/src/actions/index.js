...
// fetch surveys from the backend server
export const getSurveys = () => {
	return async (dispatch) => {
		const response = await axios.get('https://emailyserver.run-us-west2.goorm.io/api/surveys', {
			withCredentials: true,
		});
		dispatch({
			type: FETCH_SURVEYS,
...
export const deleteSurvey = (surveyId) => {
	return async (dispatch) => {
		...
		await axios.delete(`https://emailyserver.run-us-west2.goorm.io/api/surveys/delete/${surveyId}`, {
			withCredentials: true,
		});
		dispatch({
			type: DELETE_SURVEY,
